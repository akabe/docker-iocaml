...
function build_image() {
    ...
    docker build -t akabe/iocaml:$TAG dockerfiles/$TAG
    docker history akabe/iocaml:$TAG
}
...
function deploy_image() {
	...
	docker push akabe/iocaml:$TAG

	for t in $ALIAS; do
	    yellow_echo "[Deploy] Deploy image tag $TAG as $t"
	    docker tag akabe/iocaml:$TAG akabe/iocaml:$t
	    docker push akabe/iocaml:$t
	done
    else
